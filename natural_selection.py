...
choice = st.radio("Select one option for mating method. Deterministic expectations just means it goes off of pure, pre-computed probabilities, whereas stochastic mating (MUCH slower) takes a sample of the males and females, shuffles them, mates them, and then scales to generalize to the full population.", options)
list_of_counts = []
final_list = []
aa_list = []
Aa_list = []
...
if st.button('Generate animation'):
    ...
    with st.expander("Your graph is being generated. This may take up to 20 seconds. Please be patient! In the mean time, here's a non-technical overview of the algorithm used in this model."):
        st.write(""" This interactive app simulates moth populations by letting users set the starting numbers of three genotypes—aa (light), Aa (dark but carrying a light allele), and AA (dark)—and adjust their survival rates against predation. Each year, the population reproduces according to Mendelian rules, loses individuals to predators, and sheds its oldest cohort after four years. The app then generates both an animation and line graphs to show how the groups change over time, allowing students to explore how survival differences and inheritance affect genotype frequencies. While simplified—it treats Aa and AA as identical, assumes fixed survival rates, and ignores mutation or migration—it’s a fun model that shows how initial conditions and selective pressures can lead to very different evolutionary outcomes.""")
